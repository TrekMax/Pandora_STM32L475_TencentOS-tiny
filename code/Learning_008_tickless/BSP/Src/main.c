...
void task_entry_demo(void *arg)
{
    ...
    systick = tos_millisec2tick(200);
    printf("[%d|%s] %d millisec equals to %lld ticks\r\n", __LINE__, __FUNCTION__,
           200, systick);

    time_ms = tos_tick2millisec(1000);
    printf("[%d|%s] %lld tick equals to %d millisec\r\n", __LINE__, __FUNCTION__,
           (k_tick_t)1000, time_ms);
    ...
    printf("[%d|%s] before slepp, sleep systick is %lld\r\n", __LINE__, __FUNCTION__,
           systick);
    printf("[%d|%s] after %d ms, systick is %lld\r\n", __LINE__, __FUNCTION__,
           2000, after_systick);

    printf("[%d|%s] milliseconds sleep is about: %d\r\n", __LINE__, __FUNCTION__,
